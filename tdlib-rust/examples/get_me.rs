// cargo run --example get_me --features default
// cargo run --example get_me --features download-tdlib
// cargo run --example get_me --features pkg-config
            ...
            AuthorizationState::WaitEmailCode(_x) => {
                ...
                let response = functions::check_authentication_email_code(
                    enums::EmailAddressAuthentication::Code(
                        tdlib_rust::types::EmailAddressAuthenticationCode { code },
                    ),
                    client_id,
                )
                .await;
...
async fn main() {
    ...
    let client_id = tdlib_rust::create_client();
    ...
    let handle = tokio::spawn(async move {
        while run_flag_clone.load(Ordering::Acquire) {
            if let Some((update, _client_id)) = tdlib_rust::receive() {
